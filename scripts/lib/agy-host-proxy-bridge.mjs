...
export const AGY_HOST_PROXY_DOMAIN = "cc-suite-agy-host.internal";
export const AGY_HOST_PROXY_NETWORK = "default";
export const AGY_HOST_PROXY_REDIRECT_IPV4 = "203.0.113.113";
...
export function buildAgyHostProxyBridgeConfig({ dnsList, scutilProxy } = {}) {
  const domains = String(dnsList || "")
    .split("\n")
    .map((value) => value.trim())
    .filter(Boolean);
  if (!domains.includes(AGY_HOST_PROXY_DOMAIN)) {
    throw bridgeError(
      "AGY_R15_HOST_PROXY_DOMAIN_REQUIRED",
      "Apple localhost redirect domain is not installed"
    );
  }
  return {
    domain: AGY_HOST_PROXY_DOMAIN,
    redirectIPv4: AGY_HOST_PROXY_REDIRECT_IPV4,
    ...parseScutilProxy(scutilProxy),
  };
}

export function auditAgyHostProxyContainerInventory(rawJson) {
  try {
    ...
    // Malformed runtime output is not evidence of an empty inventory.
  }
  return {
    ready: false,
    findings: ["AGY_R15_HOST_PROXY_RUNTIME_NOT_EMPTY"],
  };
}

export function buildAgyHostProxyRedirectUrl(listenPort) {
  if (!Number.isInteger(listenPort) || listenPort < 1024 || listenPort > 65535) {
    throw bridgeError(
      "AGY_R15_HOST_PROXY_LISTEN_INVALID",
      "host proxy bridge port is invalid"
    );
  }
  return `http://${AGY_HOST_PROXY_REDIRECT_IPV4}:${listenPort}`;
}

export function buildAgyHostProxyProbeArgs({
  proxyName,
  redirectIPv4,
  redirectPort,
} = {}) {
  if (
    ...
    throw bridgeError(
      "AGY_R15_HOST_PROXY_PROBE_SCOPE_INVALID",
      "host proxy probe scope is invalid"
    );
  }
  return [
    "exec",
    ...
    "/usr/local/bin/node",
    "-e",
    HOST_PROXY_PROBE_SOURCE,
    redirectIPv4,
    String(redirectPort),
  ];
}

export async function runAgyHostProxyProbe(options = {}) {
  const {
    args,
    environment,
  ...
  let expectedArgs;
  try {
    expectedArgs = buildAgyHostProxyProbeArgs({
      proxyName: args?.[1],
      redirectIPv4: args?.[5],
  ...
  if (
    ...
    throw bridgeError(
      "AGY_R15_HOST_PROXY_PROBE_SCOPE_INVALID",
      "host proxy probe execution scope is invalid"
    );
  }
...
export async function createAgyHostProxyBridge(options = {}) {
  const {
  ...
  if (
    ...
    throw bridgeError(
      "AGY_R15_HOST_PROXY_SETTINGS_INVALID",
      "host proxy bridge settings are invalid"
    );
  }
  ...
  const server = net.createServer((clientSocket) => {
    ...
    if (remoteAddress === null || remoteAddress !== expectedClientIPv4) {
      logger({ event: "agy_host_proxy_denied", reason: "client_ipv4_mismatch" });
      clientSocket.destroy();
      return;
    }
    if (activeTunnels >= MAX_TUNNELS) {
      logger({ event: "agy_host_proxy_denied", reason: "concurrency_limit" });
      clientSocket.destroy();
      return;
    }
    ...
    const timer = setTimeout(() => {
      logger({ event: "agy_host_proxy_failed", reason: "connect_timeout" });
      finish();
    }, CONNECT_TIMEOUT_MS);
    timer.unref?.();
    ...
    try {
      ...
      upstreamSocket.once("connect", () => {
        ...
        logger({ event: "agy_host_proxy_tunnel_opened" });
      });
      upstreamSocket.once("error", () => {
        clearTimeout(timer);
        logger({ event: "agy_host_proxy_failed", reason: "upstream_connect_failed" });
        finish();
      });
  ...
  if (!Number.isInteger(listenPort) || listenPort < 1024 || listenPort > 65535) {
    ...
    throw bridgeError(
      "AGY_R15_HOST_PROXY_LISTEN_INVALID",
      "host proxy bridge failed to bind an ephemeral loopback port"
    );
  }
  logger({ event: "agy_host_proxy_ready" });
  ...
  return {
    ...
    authorizeClientIPv4(value) {
      if (!privateIPv4(value)) {
        throw bridgeError(
          "AGY_R15_HOST_PROXY_CLIENT_INVALID",
          "host proxy bridge client IPv4 is invalid"
        );
      }
      if (expectedClientIPv4 !== null && expectedClientIPv4 !== value) {
        throw bridgeError(
          "AGY_R15_HOST_PROXY_CLIENT_LOCKED",
          "host proxy bridge client IPv4 is already locked"
        );
      }
    ...
    async close() {
      ...
      logger({ event: "agy_host_proxy_stopped" });
    },
  };
}
