...
import { auditAgyR16ProxyPolicy } from "./lib/agy-r16-proxy-policy.mjs";
...
function parseArguments(argv) {
  if (
    argv.length !== 8 ||
    argv[0] !== "--policy-env" || argv[1] !== "AGY_PROXY_POLICY" ||
    argv[2] !== "--expected-listen-host" ||
    argv[4] !== "--expected-client-ipv4" ||
    argv[6] !== "--expected-gateway-ipv4"
  ) {
    throw proxyError("AGY_PROXY_USAGE", "R16 proxy arguments are invalid");
  }
  return {
    expectedListenHost: argv[3],
    expectedClientIPv4: argv[5],
    expectedGatewayIPv4: argv[7],
  };
}
...
async function main() {
  ...
  if (typeof rawPolicy !== "string" || rawPolicy.length < 2 || rawPolicy.length > 64 * 1024) {
    throw proxyError("AGY_PROXY_POLICY_ENV_INVALID", "R16 proxy policy is invalid");
  }
  let parsedPolicy;
  try {
    parsedPolicy = JSON.parse(rawPolicy);
  } catch {
    throw proxyError("AGY_PROXY_POLICY_JSON_INVALID", "R16 proxy policy JSON is invalid");
  }
  const auditedPolicy = auditAgyR16ProxyPolicy(parsedPolicy, {
    expectedListenHost: arguments_.expectedListenHost,
    expectedClientIPv4: arguments_.expectedClientIPv4,
    expectedGatewayIPv4: arguments_.expectedGatewayIPv4,
    expectedUpstreamUrl: parsedPolicy?.upstream?.url,
  });
  if (!auditedPolicy.ready) {
    throw proxyError("AGY_PROXY_POLICY_INVALID", "R16 proxy policy audit failed");
  }
