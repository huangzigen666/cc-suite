...
5. **Config focus instructions** — `{config_focus_instructions}` from `.cc-suite.md` Audit Focus section (if present), **audit and verification commands only** (audit, audit-fix, verify, the audit-* family). Never inject audit-focus text into implementation, planning, or debugging calls — the config defines it as audit-specific.
6. **Config project instructions** — `{config_project_instructions}` from `.cc-suite.md` Project-Specific Instructions section (if present) — every command.

Parts 1–3 are always present; omit 4–6 if empty or (for 5) out of scope. The final prompt is: `{preamble}\n\n{command-specific prompt}`.
...
>
> **The "by any route" sentence exists because the skill list alone failed a real test.** 2026-09-19, with a real `claude-code` MCP server live in `.mcp.json` (the same one Codex, agy, and CodeBuddy all reach through claude-octopus) and only the skill-naming sentence present, Codex — asked directly to "hand this task back... through that server" — actually called `mcp__claude-code__claude_code`. The call failed, but only because claude-octopus's own server rejected it with `403 Request not allowed`; Codex itself made no attempt to refuse. The other backends' boundary text (`scripts/lib/delegation-boundary.mjs`) already closes this with a route-agnostic sentence for exactly this reason — Codex's preamble didn't have the equivalent until this fix.
