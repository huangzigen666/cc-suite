...
**The reverse channel is live automatically, via `--mcp-config`, not bare `.mcp.json` discovery.** CodeBuddy does read a project's `.mcp.json` on its own in ACP mode, but a server merely *discovered* that way lands in a cautious `Disabled (ask the user to enable via /mcp)` state with no headless way to approve it — reproduced across multiple fresh projects, including ones covered by CodeBuddy's own `trustedDirectories: ["/Users/**"]` setting, so it isn't a trust gate either. The fix: `scripts/codebuddy-runner.mjs` passes `--mcp-config <cwd>/.mcp.json` on launch whenever that file exists — a server named explicitly at launch this way connects immediately (verified: `ready: true`, all 6 `claude-code` tools live, no gate), while the ACP protocol's own `mcpServers` field stays `[]` in both `session/new` and `session/load`, unused, by design. `--mcp-config` is additive (not exclusive, unless paired with `--strict-mcp-config`, which this runner doesn't set) and a safe no-op when `.mcp.json` doesn't exist. The anti-handback delegation boundary still holds with the channel genuinely live — re-verified after this fix.
